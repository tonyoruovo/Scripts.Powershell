function Get-FolderTree {
  ...
  NOTE: Symlinks to directories are not followed; a warning to that effect is
        ...
        Also, this file encoding is UTF-8 BOM.
  ...
  #>
    ...
    $ndx = [bool] $Ascii
    $chars = @{
      #Note for the character literals see https://unicodelookup.com/ and use the search bar
      interior = ([string]([char]0x251C), '+')[$ndx]
      last = ([string]([char]0x2514), '\')[$ndx]
      hline = ([string]([char]0x2500), '-')[$ndx]
      vline = ([string]([char]0x2502), '|')[$ndx]
      space = ' '
    }
...
Get-Help Get-FolderTree -Detailed
Get-FolderTree -Path C:\WebDev\Javascript\web2 -Exclude "node_modules","client",".git" -IncludeFiles -Force > C:\WebDev\Javascript\web2\tree.txt
#endregion
